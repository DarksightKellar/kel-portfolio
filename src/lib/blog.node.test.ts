...
describe("getAllPosts", () => {
  it("returns an empty array when no posts exist", async () => {
    const posts = await getAllPosts(testDir);

    expect(posts).toEqual([]);
  });

  it("returns summaries of all posts sorted by date descending", async () => {
    writePost(
      "first-post",
  ...
  it("extracts title, slug, date, and excerpt from each post", async () => {
    writePost(
      "test-post",
  ...
  it("uses the first 200 characters as excerpt when no paragraph break exists", async () => {
    const longLine =
...
describe("getPostBySlug", () => {
  it("returns null when no post matches the slug", async () => {
    const post = await getPostBySlug(testDir, "nonexistent");

    expect(post).toBeNull();
  });

  it("returns the full post with HTML content when slug matches", async () => {
    writePost(
      "my-post",
    ...
    const post = await getPostBySlug(testDir, "my-post");
